 ...
 */
wildpad.textPiecesToInserts = function(atNewLine, textPieces) {
  var inserts = [];

  function insert(string, attributes) {
    if (string instanceof wildpad.Text) {
  ...
  function insertLine(line) {
    ...
    if (atNewLine) {
      insert(wildpad.sentinelConstants.LINE_SENTINEL_CHARACTER, line.formatting.attributes);
    }
  ...
  for(var i = 0; i < textPieces.length; i++) {
    if (textPieces[i] instanceof wildpad.Line) {
