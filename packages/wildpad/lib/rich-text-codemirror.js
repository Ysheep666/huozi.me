...
wildpad.RichTextCodeMirror = (function () {
  var AnnotationList = wildpad.AnnotationList;
  ...
  var LineSentinelCharacter   = wildpad.sentinelConstants.LINE_SENTINEL_CHARACTER;
  var EntitySentinelCharacter = wildpad.sentinelConstants.ENTITY_SENTINEL_CHARACTER;
  ...
  RichTextCodeMirror.prototype.insertEntityAt = function(index, type, info, origin) {
    var cm = this.codeMirror;
    this.insertEntity_(index, new wildpad.Entity(type, info), origin);
  };
  ...
  RichTextCodeMirror.prototype.getClassNameForAttributes_ = function(attributes) {
    ...
    for (var attr in attributes) {
      ...
      if (attr === ATTR.LINE_SENTINEL) {
        wildpad.utils.assert(val === true, "LINE_SENTINEL attribute should be true if it exists.");
      } else {
  ...
  RichTextCodeMirror.prototype.markEntity_ = function(annotationNode) {
    ...
    var entity = wildpad.Entity.fromAttributes(attributes);
    var cm = this.codeMirror;
    var self = this;
    ...
    annotationNode.attachObject({
      ...
      update: function(info) {
        var entity = wildpad.Entity.fromAttributes(info);
        for(var i = 0; i < markers.length; i++) {
  ...
  RichTextCodeMirror.prototype.markLineSentinelCharactersForChanges_ = function(changes) {
    ...
    // But In practice, people using wildpad for rich-text editing don't batch multiple changes
  ...
  RichTextCodeMirror.prototype.updateCurrentAttributes_ = function() {
    ...
    if (spans.length > 0 && (!(ATTR.LINE_SENTINEL in spans[0].annotation.attributes))) {
      ...
      wildpad.utils.assert(!(ATTR.LINE_SENTINEL in spans[1].annotation.attributes), "Cursor can't be between two line sentinel characters.");
  ...
  RichTextCodeMirror.prototype.getLineAttributes_ = function(lineNum) {
    ...
    if (line.length > 0 && line[0] === LineSentinelCharacter) {
      ...
      wildpad.utils.assert(spans.length === 1);
