...
/**
 * Helper to parse html into Wildpad-compatible lines / text.
 * @type {*}
 */
wildpad.ParseHtml = (function () {
  ...
  function ParseState(opt_listType, opt_lineFormatting, opt_textFormatting) {
    ...
    this.lineFormatting = opt_lineFormatting || wildpad.LineFormatting();
    this.textFormatting = opt_textFormatting || wildpad.Formatting();
  }
  ...
  ParseOutput.prototype.newline = function(state) {
    ...
    this.lines.push(wildpad.Line(this.currentLine, lineFormatting));
    this.currentLine = [];
  };
  ...
  function parseHtml(html, entityManager) {
    ...
    var div = (wildpad.document || document).createElement('div');
    div.innerHTML = html;
  ...
  function parseNode(node, state, output) {
    ...
    if (node.nodeType === Node.ELEMENT_NODE) {
      ...
      if (entity) {
        output.currentLine.push(new wildpad.Text(
            wildpad.sentinelConstants.ENTITY_SENTINEL_CHARACTER,
            new wildpad.Formatting(entity.toAttributes())
        ));
        return;
      }
    }
    ...
    switch (node.nodeType) {
      case Node.TEXT_NODE:
        ...
        output.currentLine.push(wildpad.Text(text, state.textFormatting));
        break;
      case Node.ELEMENT_NODE:
        ...
        switch (node.nodeName.toLowerCase()) {
          ...
          case 'ul':
            ...
            var listType = node.getAttribute('class') === 'wildpad-todo' ? LIST_TYPE.TODO : LIST_TYPE.UNORDERED;
  ...
  function parseListItem(node, state, output) {
    ...
    var listType = (node.getAttribute('class') === 'wildpad-checked') ? LIST_TYPE.TODOCHECKED : state.listType;
  ...
  function parseStyle(state, styleString) {
    ...
    for(var i = 0; i < styles.length; i++) {
      ...
      if (stylePieces.length !== 2)
        continue;
      var prop = wildpad.utils.trim(stylePieces[0]).toLowerCase();
      var val = wildpad.utils.trim(stylePieces[1]).toLowerCase();
      switch (prop) {
        ...
        case 'font-size':
          ...
          if (wildpad.utils.stringEndsWith(val, allowedValues)) {
            size = val;
          }
          ...
          break;
        case 'font-family':
          var font = wildpad.utils.trim(val.split(',')[0]); // get first font.
