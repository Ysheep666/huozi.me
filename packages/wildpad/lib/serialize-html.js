...
/**
 * Helper to turn Wilddog contents into HMTL.
 * Takes a doc and an entity manager
 */
wildpad.SerializeHtml = (function () {

  var utils      = wildpad.utils;
  var ATTR       = wildpad.AttributeConstants;
  var LIST_TYPE  = wildpad.LineFormatting.LIST_TYPE;
  var TODO_STYLE = '<style>ul.wildpad-todo { list-style: none; margin-left: 0; padding-left: 0; } ul.wildpad-todo > li { padding-left: 1em; text-indent: -1em; } ul.wildpad-todo > li:before { content: "\\2610"; padding-right: 5px; } ul.wildpad-todo > li.wildpad-checked:before { content: "\\2611"; padding-right: 5px; }</style>\n';

  function open(listType) {
    return (listType === LIST_TYPE.ORDERED) ? '<ol>' :
           (listType === LIST_TYPE.UNORDERED) ? '<ul>' :
           '<ul class="wildpad-todo">';
  }
    ...
    while(op) {
        ...
        if (listType) {
          ...
          switch (listType)
          {
            case LIST_TYPE.TODOCHECKED:
              clazz = ' class="wildpad-checked"';
              break;
            case LIST_TYPE.TODO:
              clazz = ' class="wildpad-unchecked"';
              break;
          }
          html += "<li" + clazz + style + ">";
          inListItem = true;
      ...
      if (ATTR.ENTITY_SENTINEL in attrs) {
        for(var j = 0; j < op.text.length; j++) {
          var entity = wildpad.Entity.fromAttributes(attrs);
      ...
      if (newLineIndex >= 0) {
        ...
        if (newLineIndex < text.length - 1) {
          // split op.
          op = new wildpad.TextOp('insert', text.substr(newLineIndex+1), attrs);
        } else {
          op = doc.ops[++i];
        }
